...
$users = gwmi win32_userprofile | select localpath, sid | Where-Object { $($_.localpath) -like 'C:\Users\*' }

foreach ($user in $users) { 
                          $taskbaricons = Get-ChildItem -Path "$($user.localpath)\AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar"
                          if ($taskbaricons.Name -eq "Command Prompt.lnk"){Set-PinnedApplication -Action PintoTaskbar -FilePath "C:\Program Files (x86)\Microsoft Office\Office15\WINWORD.exe"
                          }
                          }
