...
$SubFolders = (Invoke-GenericMethod `
    ...
    -MethodParameters   "\\hq-san-01\endeavor", '*',
                        ...
                        ([Alphaleonis.Win32.Filesystem.PathFormat]::FullPath)) | where {($_.Filename -eq "fcs") -or ($_.Filename -eq "Departments") -or ($_.Filename -eq "media") -or ($_.Filename -eq "Projects") -or ($_.Filename -eq "ProPricer")}
#----------------------------------------------------------
...
foreach ($SF in $SubFolders){
    ...
    Start-Job -Name "Endeavor Restore $($SFFullPath)" -ScriptBlock {
        $path = "C:\Users\sd_Ashuttleworth\Desktop\Endeavor\Restore\Final\"
        $logSuccess     = $path + "Endeavor_Final_Success_$($args[1]).log"
        $logError       = $path + "Endeavor_Final_Error_$($args[1]).log"
        $logRestore     = $path + "Endeavor_Final_Restore_$($args[1]).log"
        ...
        "STARTED (on " + $StartDate + "): " | Out-File $logRestore -append
    ...
    } -InitializationScript { Import-Module -Name "C:\Windows\System32\WindowsPowerShell\v1.0\Modules\AlphaFS.2.0.1\lib\net451\AlphaFS.dll"
        Function Invoke-GenericMethod {
            Param(
                $Instance,
                [String]$MethodName,
