...
$SubFolders = (Invoke-GenericMethod `
    ...
    -MethodParameters   "\\hq-san-01\endeavor", '*',
                        ...
                        ([Alphaleonis.Win32.Filesystem.PathFormat]::FullPath)) | where {($_.Filename -eq "fcs") -or ($_.Filename -eq "Departments")}
#----------------------------------------------------------
...
{   $NewDir = New-Item -Path "C:\Users\sd_Ashuttleworth\Desktop\Endeavor\Restore\" -Name $($SF.Filename) -ItemType directory -ErrorAction SilentlyContinue
    ...
    Start-Job -Name "Endeavor Restore $($SFFullPath)" -ScriptBlock {
        $path = "C:\Users\sd_Ashuttleworth\Desktop\Endeavor\Restore\$($args[1])\"
        $logSuccess     = $path + "Endeavor_Copy2_Success_$($args[1]).log"
        $logError       = $path + "Endeavor_Copy2_Error_$($args[1]).log"
        $logRestore     = $path + "Endeavor_Copy2_Restore_$($args[1]).log"
        $log            = $path + "Endeavor_Copy2_archiveditems_$($args[1]).log"
