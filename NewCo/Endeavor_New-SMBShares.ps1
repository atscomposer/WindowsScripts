...
$csv| ForEach-Object {$path = "E:" + $_.Path
                         ...
                         <#if (!(Get-SmbShare $_.Name)){
                          Write-Host $path}}#>
                          #New-SmbShare -Name $_.Name -ScopeName EDR-ATTIC -ContinuouslyAvailable $true -FolderEnumerationMode AccessBased -Path $PATH -ReadAccess "Endeavor Robotics Attic Access"
                          Grant-SmbShareAccess -Name $_.Name -ScopeName EDR-ATTIC -AccountName "Domain Admins" -AccessRight Full -Confirm:$false}
