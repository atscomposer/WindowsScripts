...
Foreach ($computer in $projremovecsv) {Remove-CMDeviceCollectionDirectMembershipRule -CollectionID HQ1000D2 -ResourceID $(get-cmdevice -Name $computer).ResourceID -Force -Confirm:$false}
Foreach ($computer in $projremovecsv) {Add-CMDeviceCollectionDirectMembershipRule -CollectionID HQ1000C9 -ResourceID $(get-cmdevice -Name $computer).ResourceID}
